...
### not yet looked up license of right ###
        ...
        xml.patent { |p| p.application_number(application_number); p.publication_number (publication_number); p.filing_date(filing_date); p.status(status); p.application_title(application_title); p.last_renewal_date(last_renewal_date); p.next_renewal_date(next_renewal_date); p.last_renewal_year(last_renewal_year) }
        #Applicant removed from xml as don't yet know how to deal with the multi-line response that arrives
    end
